...
//     apiDiv.innerText = dataTransformed.results.length[].name;
// };
// contactApi();
...
const contactApiSecurePlus =  async () => {
    try {
        ...
        console.log(transformedData);
        for(let i = 0; i < 20; i++){
            apiDiv.innerHTML = apiDiv.innerHTML + '<p>' + transformedData.results[i].name + '</p>';
            // Autre méthode
            // apiDiv.innerHTML += '<p>' + transformedData.results[i].name + '</p>';
        }
    } catch (error) {
        console.error("Erreur lors de l'appel à l'API : ", error);
    }
